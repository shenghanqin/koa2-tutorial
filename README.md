...
# 整理思路
确定需求后，我们就清楚需要做的事情。现在，从一个请求进入 `Koa` 开始说起：
...
可以看到，我们的关键点是捕捉错误及实现错误处理逻辑和渲染页面逻辑。
...
文件的读取和渲染在 `consolidate` 完成，它是一个模板引擎统一库，几乎支持所有的模板引擎。由此，需要提前引入该模块。另外，因为牵涉到文件路径的解析，我们还需要引入 `path` 模块。
